...
Ty = 23; Tz = 30; %Ty 31.5572~200 对应 1.9THz~0.3THz ; 16.9990669701 THz 对应 θy =0.00010745 °出射...
...
tau_L = 200 ;%设置飞秒 脉冲 时域波形 脉宽，单位飞秒
...
gy = 2*pi/(Ty*(my))*(-my/2:my/2-1);   %倒空间坐标ky
gz = 2*pi/(Tz*(mz))*(-mz/2:mz/2-1);   %倒空间坐标kz

sety = 9; %设置y向匹配倒格矢位置
disp(['设置 y 向细节倒格矢：' num2str(gy(sety))]); % ky(6)：y方向上的第六个以调制区域长度为周期的倒格矢，转换为字符串，并输出打印
...
if ny<0
    disp(['计算 Ty, Tz 理论最小值：' 'Ty_min = ' num2str(ny*2*pi/(-sqrt(max(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2)-Gz^2)-gy(sety))) ' μm' ', Tz_min = ' num2str(abs(nz)*2*pi/max(sqrt(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2))) ' μm']);
else
    disp(['计算 Ty, Tz 理论最小值：' 'Ty_min = ' num2str(ny*2*pi/(sqrt(max(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2)-Gz^2)-gy(sety))) ' μm' ', Tz_min = ' num2str(abs(nz)*2*pi/max(sqrt(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2))) ' μm']);
end
%让 Ty 对 ny 的正负 不敏感。
...
index1 = find(abs((set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2-Gz^2)-(gy(sety)+Gy)^2)==min(abs((set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2-Gz^2)-(gy(sety)+Gy)^2)));
...
r = sqrt(X.^2+Y.^2+Z.^2);
% - - - - - - - - - - - - - - - - - -
gx = 2*pi/(Tx*(mx))*(-mx/2:mx/2-1);   %倒空间坐标kx 用B-K边界条件，认为正空间中15个Tx长度为一个周期，延拓，所得的倒格矢分布，取值也是-8~7，注意与Gx不同，Gx的周期是Tx，而这里的周期是15*Tx，有点像格波的波矢，比较小。
% - - - - - - - - - 
...
% SZ = -Gz; %倍频波k2在z方向分量
% SR = k2; %倍频波k2的大小
% [gx_surf,gz_surf] = meshgrid(gx,gz); %用gx,gz两个一维数组，即调制区域的倒格矢，生成一个x-o-z平面的二维网格
% ysurf2 = k_^2 - (gx_surf-SX).^2 - (gz_surf-SZ).^2; %k2^2-(k2_x-kx)^2-(k2_z-kz)^2，其中k2的起点与ky的起点相同   %-(k2_y-ky)^2，其中因选了第六个面，而使得set_gy≈ky(6)。
% gy_surf = sqrt(ysurf2.*(ysurf2>=0))+SY;  %计算出相位匹配球面，下一步需要找出离散倒格矢中的相近位置，当ysurf2大于等于零时， ysurf = sqrt(ysurf2)+k2_y，否则等于 k2_y
...
for ii = 1:mx
    for jj = 1:mz
        [~,index_ysurf] = sort(abs(gy_surf(ii,jj)-gy)); %忽略sort输出的第一个参数，将接近ysurf(ii,jj)的ky升序排列地筛选出来，不一定是ky(6)，可能是ky(7)、ky(8)，筛选出来后储存在一维数组index_ysurf中，索引号有小到大分别对应筛选出来的距离球面由远及近的ky们，即第几层们
