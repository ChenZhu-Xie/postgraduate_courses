...
Tx = 0.6; Tz = 4; Ty = 60;   %实空间像素尺寸，即每一个小格子的尺寸、三维周期每个维度的周期
%采样 
...
set_gy = set_k_THz*cos(theta_y)+Gy; % y向失配量；倍频波波矢k_3（这里是k_2）曲面被目标级次的倒格矢匹配完成后，取k_3在y轴方向的分量大小，减去两倍的基波波矢，再减去相应级次的G_y，获得该方向的差距，即倒格矢补偿后，仍还有的、剩下的y方向的波矢失配量！
