...
Ty = 22.5; Tz = 21.1; %Ty 31.5572~200 对应 1.9THz~0.3THz ; 16.9990669701 THz 对应 θy =0.00010745 °出射...
...
Gzplusgzmo_max = max(sqrt(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2));
Tz_min = abs(nz + lz/mz)*2*pi/Gzplusgzmo_max;
Ty_min = 0;
if abs(Gz + gz) < Gzplusgzmo_max
    if abs(Gz + gz) < min(sqrt(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2))
        Gyplusgymo_min = min(sqrt(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2)) - abs(Gz + gz);
        Ty_max = abs(ny + ly/my)*2*pi/Gyplusgymo_min;
    end
...
if abs(Gz + gz) < min(sqrt(set_k_THz_LiNbO3.^2-set_k_THz_LiNbO3_x.^2))
    disp(['计算 Ty, Tz 匹配范围（需首先满足 Tz）：' 'Ty ∈ ' num2str(Ty_min) ' ~ ' num2str(Ty_max) ' μm' ', Tz ∈ ' num2str(Tz_min) ' ~ ' '∞' ' μm']);
else
    disp(['计算 Ty, Tz 匹配范围（需首先满足 Tz）：' 'Ty ∈ ' num2str(Ty_min) ' ~ ' '∞' ' μm' ', Tz ∈ ' num2str(Tz_min) ' ~ ' '∞' ' μm']);
end
...
if (Gy + gy)^2 > k_THz_match_LiNbO3^2 - k_THz_match_LiNbO3_x^2 - (Gz + gz)^2
    if index1 == length(set_v_THz)
        disp(['匹配 THz 波长、频率：' 'λ_THz = ' num2str(lam_THz_match) ' μm' ', ν_THz = ' num2str(v_THz_match) ' THz' ' ———— 请减小 脉冲脉宽，以提高 THz包络 中心频率；或 增大 Ty, Tz 以减小匹配倒格矢']);
    else % 不必像下面这样严格要求 G 所提供的倒格矢 要在 太赫兹波矢曲面内。之前之所以要求 G 的部分分量在球面内，其实是要求其小于球的半径，以给其他 G 的分量提供 匹配空间。
    ...
    if index1 == 1
        disp(['匹配 THz 波长、频率：' 'λ_THz = ' num2str(lam_THz_match) ' μm' ', ν_THz = ' num2str(v_THz_match) ' THz' ' ———— 请增大 脉冲脉宽，以降低 THz包络 中心频率；或 减小 Ty, Tz 以增大匹配倒格矢']);
    else
        disp(['匹配 THz 波长、频率：' 'λ_THz = ' num2str(lam_THz_match) ' μm' ', ν_THz = ' num2str(v_THz_match) ' THz']);
    end
end
...
Gx_plus_gx = k_THz_match * n_g(lam_inc) - k_THz_match_LiNbO3_x;
disp(['计算 晶体内 kT 及 kg：' 'kT = ' num2str(k_THz_match_LiNbO3) ', kg = ' num2str(k_THz_match * n_g(lam_inc))]);
Tx = (nx + lx/mx)*2*pi/Gx_plus_gx;

Tz = Tx;% 方形小孔
w = Tx/4 ; t = Ty/2 ; h = Tz ; %小孔高度为 w（即q_mn）、小孔宽度为 h（即p_mn）
%disp(['匹配 结构 x 向周期，设置 结构 y, z 向周期：' 'Tx = ' num2str(Tx) ', Ty = ' num2str(Ty) ', Tz = ' num2str(Tz)]);
L_eff = mx*Tx ; B_eff = my*Ty ; A_eff = mz*Tz ; %调制区域尺寸

% 有了周期后才能算实空间x坐标、细节倒格矢
x = Tx*(-floor(mx/2):floor((mx-1)/2));  %实空间坐标x 序号为-8~7，x是一个一维数组，每一个x的具体值，对应某一个最小周期结构的x边界，因含Tx而有单位
set_gx = 2*pi/(Tx*(mx))*(-floor(mx/2):floor((mx-1)/2)); %倒空间坐标kx 用B-K边界条件，认为正空间中15个Tx长度为一个周期，延拓，所得的倒格矢分布，取值也是-8~7，注意与Gx不同，Gx的周期是Tx，而这里的周期是15*Tx，有点像格波的波矢，比较小。
...
if Tx > 0
    ...
    disp(['匹配 结构 x 向周期：' 'Tx = ' num2str(Tx) ' μm' ' ———— x 向无法完成匹配！ 请选择 相反 mx + lx 级次 的 Gx + gx 以弥补 x 向波矢失配！']);
end
...
Gyplusgy_surf2 = k_THz_match_LiNbO3^2 - (k_THz_match * n_g(lam_inc) - Gx - gx_surf).^2 - (Gz + gz_surf).^2;
panduan = 1*(Gyplusgy_surf2>=0);
if Gy + gy > 0 
    gy_surf = sqrt(Gyplusgy_surf2.*panduan) - Gy;
