...
theta_x=theta_x_du/180*pi;
theta_y=theta_y_du/180*pi;
...
%计算匹配波长-细节倒格矢曲线
set_lam_inc = 1.3;
set_v_THz = 0.3:0.001:1.9; %THz
set_lam_THz = c./(set_v_THz*10^12)*10^6; % 以0.01为增量，从0.5增加到1.5，得到一个维数组，单位可能是微米
%set_lam2 = set_lam1/2; % 上一个除以2，即倍频波的波长，会短一半
set_k_THz = n_THz(set_lam_THz)*2*pi./set_lam_THz; % 给定温度T 和 给定波长下的 基波波矢大小，利用了C_n.m算晶体内相应波长和温度的折射率，其中C_n就是折射率，分子分母同时除以折射率，分母就是晶体内的波长
%set_k2 = C_n(set_lam2)*2*pi./set_lam2; % 倍频波在晶体中的波长大小，以及在晶体中相应的波矢大小，仍然是个一维数组
set_gy = -sqrt(set_k_THz.^2-(2*pi./set_lam_THz*n_g(set_lam_inc)-Gx).^2-Gz^2)-Gy; % y向失配量；倍频波波矢k_3（这里是k_2）曲面被目标级次的倒格矢匹配完成后，取k_3在y轴方向的分量大小，减去两倍的基波波矢，再减去相应级次的G_y，获得该方向的差距，即倒格矢补偿后，仍还有的、剩下的y方向的波矢失配量！
% figure();plot(set_gy,set_lam1);

wer1=set_k_THz.^2-(2*pi./set_lam_THz*n_g(set_lam_inc)-Gx).^2;
...
sety = 9; %设置y向匹配倒格矢位置
disp(['设置y向细节倒格矢：' num2str(ky(sety))]); % ky(6)：y方向上的第六个以调制区域长度为周期的倒格矢，转换为字符串，并输出打印
...
disp(['匹配THz波长：' num2str(lam_THz_match)]); %找到了匹配的波长所对应的序号后，将匹配的波长set_lam1(index1)展示出来。
v_THz_match=c/(set_lam_THz(index1)*10^(-6))*10^(-12);
disp(['匹配THz频率：' num2str(v_THz_match)]); %找到了匹配的波长所对应的序号后，将匹配的波长set_lam1(index1)展示出来。
